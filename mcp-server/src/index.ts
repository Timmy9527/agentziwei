  ...
  try {
    ...
    if (name === 'get_payment_link') {
      const paper = (args && typeof args === 'object' && 'paper' in args)
        ? String((args as { paper?: string }).paper || 'ming')
        : 'ming'
      const data = await apiGet(`/api/mcp/payment-link?paper=${encodeURIComponent(paper)}`) as { url: string }
      return {
        content: [{ type: 'text' as const, text: data.url || JSON.stringify(data) }],
        isError: false,
      }
    }
    return {
